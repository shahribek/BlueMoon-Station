	...
	if(ishuman(user))
		switch(activity)
			...
			if(TUMOR_INACTIVE)
				...
				var/list/candidates = pollCandidatesForMob("Do you want to play as a lavaland elite?", ROLE_SENTIENCE, null, ROLE_SENTIENCE, 50, src, POLL_IGNORE_SENTIENCE_POTION, priority_check = FALSE)
				if(candidates.len)
